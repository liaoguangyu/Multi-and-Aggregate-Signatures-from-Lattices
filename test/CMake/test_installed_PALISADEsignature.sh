...
# Build and install the PALISADEsignature library
function build_lib() {
    rm -rf ../../build
    mkdir ../../build
    pushd ../../build 
...
# Build the examples with the installed PALISADEsignature
function build_examples() {
    rm -rf build
    mkdir -p build
    pushd build
    cmake .. || exit_with_msg "Failed to config test harness"
    make -j16 || exit_with_msg "Failed to build examples with installed PALISADEsignature package"
    popd
}
build_examples
